...
- [ ] **ADR-0016 (drain lifecycle)** is written and committed in the design
...
- [ ] Config gains a top-level `reload` block with `drain_window` (pointer
...
- [ ] The proxy derives each outbound request context with cancel-with-cause
...
- [ ] A drain-cancelled request before headers: 502, slot released, WARN line
...
- [ ] Tests Red first: config load/validate cases; retiring a backend with
      ...
      `-race` clean.
- [ ] A benchmark of the proxy request path with the join; result recorded in
      the session log.
