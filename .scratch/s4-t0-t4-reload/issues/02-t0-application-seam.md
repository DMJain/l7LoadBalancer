...
- [x] `internal/app` exposes **build** (validated config + logger → application
...
- [x] The application value exposes the client handler, the metrics
...
- [x] Build performs today's wiring unchanged: collector; registry with every
      ...
      handler.
- [x] `main` becomes flags → `probe` subcommand short-circuit → load →
...
- [x] The chaos-test assembly delegates to build; every chaos test passes
      with unchanged assertions.
- [x] A build-level test proves the handler routes to configured backends and
      every backend's seeded gauge series exists.
- [x] `AGENTS.md`'s package dependency graph and directory map show
...
- [x] No ADR (package decision recorded in the PROGRESS entry and AGENTS.md).
- [x] `make test`, `make test-race`, `go vet ./...`, `make fmt` clean; manual
