...
- [ ] **ADR-0015 (reload architecture)** is written and committed in the
...
- [ ] A name kept with a changed URL yields one removed plus one added entry,
      never an "updated" one.
- [ ] A pure non-backend-change function returns the names of differing fields
...
- [ ] Table-driven tests: identical; pure add; pure remove; URL change under
...
- [ ] Neither function logs, errors, or touches runtime state.
