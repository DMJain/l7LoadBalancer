...
- [ ] The registry holds one atomic pointer to an immutable snapshot of a
      ...
      load.
- [ ] **Apply** takes a diff, builds the next snapshot in the new file's order
...
- [ ] The consistent-hash bounded-loads selector caches (version, ring) and
      ...
      static.
- [ ] The application holds an atomic pointer to the loaded config, set by
...
- [ ] Tests (Red first): unchanged instances keep pointer identity and state
...
- [ ] Hand-off recorded in the PROGRESS entry: apply is test-only until T3;
