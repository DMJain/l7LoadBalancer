...
- [x] Reload starts one drain goroutine per removed backend. It waits for
...
- [x] On completion it deletes the backend's active-connections series only if
...
- [x] Drain goroutines select on the process shutdown context and exit
      immediately without cleanup; shutdown timing is unchanged.
- [x] A drain is independent of later reloads: re-adding the same identity
      creates a fresh backend and neither revives nor ends the draining one.
- [x] **Exit test, case 1** (chaos harness, `-race`): 1000 concurrent requests
      ...
      circuit and EWMA state intact (`TestChaosReloadDrainExitCriterion1000`).
- [x] **Exit test, case 2:** drain window shorter than the hold; removed name
      ...
      (`TestChaosReloadDrainWindowExpired`).
- [x] Shutdown-during-drain test: cancelling the process context while a drain
      is running returns promptly (`TestDrainBackendShutdownExitsWithoutCleanup`).
- [x] PROGRESS entry notes the Sprint 4 exit criterion "SIGHUP with 1000
