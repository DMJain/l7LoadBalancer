...
- **New "Deviations from plan" section** — exactly two entries:
  ...
  2. **`MILESTONES.md`'s "custom Transport pattern" wording**: documented as a pre-implementation scope clarification, not a mid-implementation deviation. `sprint-1-contracts.md:115` and `.scratch/s1-t3-t9-backend-selector-proxy/spec.md:115` both explicitly deferred `http.Transport` tuning to Sprint 4 *before* S1.T6 was coded, using near-identical wording; "RoundTripper" never appears anywhere in the project's git history. `MILESTONES.md`'s original bullet (written in the initial scaffold commit, never edited since) simply was never updated to match the already-settled scope.
