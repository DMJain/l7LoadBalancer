...
- [x] `Config` gains new **global** (not per-backend) fields for: health
      ...
      declaration
- [x] Fields are optional in YAML: omitting any of them defaults to a
...
- [x] `Validate()` rejects a non-positive (zero or negative) duration for
      any of the three fields when explicitly set
- [x] `KnownFields(true)` strictness is preserved — a typo'd field name still
      fails to load
- [x] Consecutive-failure/success thresholds, outlier window size, and
...
- [x] Test: a config omitting all three new fields loads and validates,
      with the documented defaults observable
- [x] Test: a config setting explicit values for all three loads them
      correctly
- [x] Test: a config setting a non-positive value for any of the three
      fails `Validate()`
