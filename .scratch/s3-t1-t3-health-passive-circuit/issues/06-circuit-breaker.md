...
- [x] Circuit state (closed/open/half-open enum, consecutive-failure count,
      ...
      reverse)
- [x] `internal/circuit` holds the policy: consecutive-failure-to-open
...
- [x] Circuit's failure counter is **consecutive** — reset to zero on any
...
- [x] Open→Half-Open promotion is lazy and timer-free: any read of circuit
      state checks `time.Since(openedAt) >= cooldown` and CASes
      `Open`→`HalfOpen` on the spot; no background goroutine
- [x] `main.go` constructs the circuit policy and registers it via
      `Proxy.RegisterObserver` (02's mechanism)
- [x] `ServeHTTP` calls `Allow(b)` immediately after `Select()` returns and
...
- [x] `Registry.Healthy()` renamed to `Registry.Selectable()`, filtering on
...
- [x] A half-open backend stays fully `Selectable()` — no pool-membership
      exclusion during half-open; only `Allow()` gates the trial
- [x] The half-open trial slot uses a real `CompareAndSwap`, not a plain bool
- [x] A trial resolves to Closed on a single success, back to Open on a
      single failure — no additional threshold inside half-open
- [x] Test: N consecutive failures opens the circuit; a single success from
      Closed resets the consecutive-failure count to zero
- [x] Test: `Registry.Selectable()` excludes an open-circuit backend and
      includes a half-open one
- [x] Test: after the configured cooldown elapses (real `time.Sleep`, ~50ms
      cooldown / ~200ms sleep margin — no fake clock), the circuit is
      observed as half-open
- [x] Test: `Allow()` admits exactly one concurrent trial during half-open
      and denies a second concurrent one
- [x] Test: a successful trial closes the circuit; a failed trial reopens it
- [x] Test: `Allow()` denying a request never increments `ActiveConns`
- [x] Test: injecting a run of 5xx responses on one backend opens its
