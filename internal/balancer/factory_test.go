...
// must refuse: the Sprint 2 identifiers (whose constructors still panic),
// an arbitrary unknown string, and the empty string. Empty is included
// deliberately — config.Validate normalizes it to round_robin, so an empty
// Algorithm reaching the factory means the caller skipped validation, and a
// silent default here would mask that.
func TestNewFromConfigRejectsUnsupportedAlgorithm(t *testing.T) {
	tests := []struct {
		name      string
		algorithm string
	}{
		{name: "consistent_hash (Sprint 2)", algorithm: config.AlgorithmConsistentHash},
