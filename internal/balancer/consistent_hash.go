...
// healthy backends. 0.25 is the value cited in Mirrokni-Thorup-Zadimoghaddam
// (2016) and is a Go constant, not a config field — no operator has asked to
// tune it. See ADR-0009.
...
// over currently-healthy backends.
//
...
//
// When no healthy backend exists it returns ErrNoHealthyBackends, the single
// condition every selector in this package uses — never a distinct
// "over capacity" error, because the capacity floor and the healthy-set
...
// Select implements Selector. It computes the current capacity across healthy
// backends once per call, then walks the ring from the request's hash key and
// returns the first candidate that is healthy and not over capacity. It never
// mutates ActiveConns — like LeastConnections, it only reads it; connection
// bookkeeping is the proxy's job.
//
...
// branch is defensive and unreachable with a non-empty healthy set: the mean
...
// max(1, ceil(avg_active * (1 + capacityEpsilon))), averaged over healthy.
// The floor of 1 keeps an idle system (avg_active = 0) from rejecting every
// backend's first request against a zero-sized cap.
func capacityFor(healthy []*backend.Backend) int64 {
	var total int64
