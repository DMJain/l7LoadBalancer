...
| [0015](docs/adr/0015-reload-architecture.md) | Zero-downtime reload architecture: in-process snapshot swap, backend identity, and admission | Accepted |
| [0016](docs/adr/0016-drain-lifecycle.md) | Drain lifecycle: retired-context join, two-phase drain, and cancel-at-window | Accepted |
...
22. **Reload swaps an immutable versioned backend snapshot in process; identity is `(name, URL)` and only the backend list is reloadable** — ADR-0015. `Registry` holds one atomic pointer to an immutable `(version, ordered backends)` snapshot read in a single load; `apply` is the single writer, reuses the existing instance for each unchanged identity, constructs fresh for added, and **marks removed backends before the swap** via a new method-only unexported `Backend` removed flag (amending ADR-0002 decision 5). A name with a changed URL is one removed plus one added, never an "updated"; new order follows the new file. The proxy skips the whole observer fan-out for a removed backend (active-connection accounting unaffected), the consistent-hash ring rebuilds on a snapshot-version mismatch (pull-based, CAS'd), and a non-backend change rejects the reload whole. Added backends start unhealthy and are admitted by one successful probe with reason `initial_probe` (startup backends start healthy — a deliberate asymmetry), so a blue/green reload has a logged WARN window with nothing selectable; draining (ADR-0016) is separate.
23. **Drain cancellation is a per-backend retired context joined into each request with `context.AfterFunc`; the bucket is config while the cut-off signal is structural** — ADR-0016. `reload.drain_window` (pointer duration, default 30s, zero/negative rejected, non-reloadable and named by `NonBackendChanges`) is the bound. `Backend` gains an unexported retired context created at construction, reachable only via `Retire()` and `RetiredContext()` (method-only, amending ADR-0002 decision 5), whose cancellation carries `ErrDrainWindowExpired`. `proxy.ServeHTTP` derives the outbound context with `context.WithCancelCause` and registers one `context.AfterFunc` on the backend's retired context, stopped on request completion — no goroutine per request, no cancel-func registry. Before headers, a drain cancellation surfaces through `ErrorHandler` as a 502 with its slot released and a WARN `reason=window_expired`, and reaches no observer because the backend was already removed (ADR-0015 decision 8); after headers it truncates the body while the success recorded at headers stands. The drain goroutine itself (S4.T4) polls `ActiveConns` and fires `Retire()` at window expiry.
