...
- [IN_PROGRESS] S3.T2 — Passive outlier detection (issue 05) (opencode, started 2026-09-20T07:27:14Z, count-based sliding window registered into the round-trip observer fan-out)
  ...
  - Test approach: detector's own `ObserveRoundTrip` called directly against real `*backend.Backend` fixtures from `backend.NewRegistry` (spec seam 4), with an unexported eject-function test seam so the invocation count is assertable (Backend.MarkUnhealthy is idempotent, so its call count is not otherwise observable); one `httptest`-backed proxy integration test registers the detector alongside a spy observer to prove it receives outcomes through the fan-out (spec seam 3). `testify/require` for setup, `testify/assert` for values; deterministic fixtures, no mocks.
